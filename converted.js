...
class Rtorrent {
  ...
  getMulticall(method, param, cmds, callback) {
    ...
    for (var c in cmds) cmdarray.push(cmds[c] + "=");
    ...
    self.getGlobals(function (err, globals) {
      ...
      self.getTorrents(function (err, torrents) {
        ...
        for (var t in torrents) {
          ...
          params.push("");
          for (var f in fields.files) params.push(fields.files[f] + "=");
        ...
        for (var t in torrents) {
          ...
          params.push(torrents[t].hash);
          params.push("");
          for (var f in fields.trackers) params.push(fields.trackers[f] + "=");
        ...
        for (var t in torrents) {
          ...
          params.push(torrents[t].hash);
          params.push("");
          for (var f in fields.peers) params.push(fields.peers[f] + "=");
        ...
        self.getXmlrpc("system.multicall", [array], function (err, data) {
          ...
          for (var i = 0; i < nb; i++) {
            torrents[i]["files"] = doublearray2hash(
              data[i][0],
              Object.keys(fields.files)
            );
            torrents[i]["trackers"] = doublearray2hash(
              data[i + nb][0],
              Object.keys(fields.trackers)
            );
            torrents[i]["peers"] = doublearray2hash(
              data[i + nb + nb][0],
              Object.keys(fields.peers)
            );
          }
  ...
  getTorrents(callback) {
    ...
    self.getMulticall("d.multicall", ["main"], fields.torrents, function (
      err,
      data
    ) {
      if (err) return callback(err);
  ...
  systemMulticall(cmds, callback) {
    ...
    this.getXmlrpc("system.multicall", [array], function (err, data) {
      ...
      var res = {};
      var i = 0;
      for (var key in cmds) res[key] = data[i++][0];
      callback(err, res);
    });
  }
...
function doublearray2hash(array, keys) {
  for (var i in array) array[i] = array2hash(array[i], keys);
  return array;
}
